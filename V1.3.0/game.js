...
function playSoundWithMul(soundName, volumeMul) {
  if (!soundEnabled) return;
  updateSoundVolumes();
  try {
    const sound = sounds[soundName];
    if(sound) {
      ...
      const masterMul = volumeSettings.master / 100;
      const soundEffectsMul = volumeSettings.soundEffects / 100;
      if(baseVolumes[soundName] !== undefined) {
        sound.volume = baseVolumes[soundName] * masterMul * soundEffectsMul * volumeMul;
      }
      const isMusic = soundName === 'background' || soundName === 'speedUp' || soundName === 'speedUpLoop';
      if(!isMusic) {
        sound.playbackRate = 1.07 + Math.random() * 0.08;
      } else {
        sound.playbackRate = 1;
      }
      sound.currentTime = 0;
    ...
    for(let b of bobbles){
      if(!b.collected && player.x + player.width > b.x && player.x < b.x + b.size && player.y + player.height > b.y && player.y < b.y + b.size){
        ...
        if(b.type === 'jumper' || b.type === 'shield') {
          ...
          playSoundWithMul('minusCollect', 0.7);
        } else if(b.type === 'healer' || b.type === 'extremeHealer') {
          playSoundWithMul('healCollect', 1);
        } else {
          playSoundWithMul('collectGem', 0.6);
        }
        ...
        switch(b.type) {
          ...
          case 'minus':
            // Minus min(10% of current score, 200)
            score = Math.max(0, score - Math.min(Math.floor(score * 0.1), 200));
            spawnParticlesEarly(b.x + b.size/2, b.y + b.size/2, "gem", runtime.effects.jumpEffectMul);
            break;
